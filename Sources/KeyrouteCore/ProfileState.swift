import Foundation

public struct ProfileState: Codable {
    public var activeProfile: String?
}

public enum StateStore {
    public static func path(environment: [String: String] = ProcessInfo.processInfo.environment) -> String {
        "\(PlatformPaths.stateDirectory(environment: environment))/keyroute/state.json"
    }

    public static func save(activeProfile: String) throws {
        let path = path()
        let url = URL(fileURLWithPath: path)
        try FileManager.default.createDirectory(at: url.deletingLastPathComponent(), withIntermediateDirectories: true)
        let data = try JSONEncoder().encode(ProfileState(activeProfile: activeProfile))
        try data.write(to: url, options: [.atomic])
    }

    public static func load() throws -> ProfileState {
        let path = path()
        guard FileManager.default.fileExists(atPath: path) else {
            return ProfileState(activeProfile: nil)
        }
        let data = try Data(contentsOf: URL(fileURLWithPath: path))
        return try JSONDecoder().decode(ProfileState.self, from: data)
    }
}
